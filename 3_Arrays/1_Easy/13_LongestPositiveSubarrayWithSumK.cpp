...
int better(vector<int>& nums, long long k){ 
    ...
    - if it does: 
    store the len as currentSumIndex(x) - remainingSumIndex(x-k), then compare it with maxlen
    ...
    */
    ...
    for(int i = 0; i < nums.size(); i++){
        ...
        // preSumMap.find() tries to find the k(sum) in the map
        ...
        if(preSumMap.find(sum) == preSumMap.end()){ // if map doesn't contain a sum, preSumMap.find(sum) will point to end()
